...
(() => {
  const { HX, HY, HS, hd, X_END, PTB, tx, price, TW0, T, SIT, camDesk, fillInk } = HI;
  const flat = HI.flat;
  ...
  const E = {
    ...
    look: wt('I2', 'Then') + .05, lock: wt('I2', 'close') - .02, gone: 62.96,
  ...
  // ---------- props ----------
  // coin() from look.js without its label, at native-fill cost (same shapes and colours).
  function coinLite(x, y, r, key) {
    if (r < 1) return;
    boilSeed('coin' + key);
    flat(ellPts(x, y + r * .09, r, r, 30, .8), C.goldDk);
    fillInk(ellPts(x, y, r, r, 30, .8), C.gold, C.ink, clamp(r / 90, .5, 1.3));
    fillInk(ellPts(x, y, r * .78, r * .78, 26, .6), mixCol(C.gold, C.goldLt, .35), C.goldDk, clamp(r / 140, .35, .9));
  }
  ...
  function jaw(x, y, s, open, t) {
    ...
    for (const sg of [1, -1]) {                                // lower jaw first, the upper one over it
      ...
      const P = [[-s * 1.1, 0], [-s * .95, sg * s * .5], [-s * .25, sg * s * .72], [s * .35, sg * s * .55], [s * .45, 0]];
      fillInk(P, sg < 0 ? '#5A4B78' : '#4B3F63', C.ink, 1.3, 255, true);
      const Tt = [[-s * 1.02, 0]]; for (let i = 0; i < 5; i++) { const x0 = -s * 1.02 + i * s * .27; Tt.push([x0 + s * .135, -sg * s * .24], [x0 + s * .27, 0]); }
      fillInk(Tt, C.cream, C.ink, .8);
      if (sg < 0) { fillInk(ellPts(-s * .3, -s * .38, s * .16, s * .19, 12), C.cream, C.ink, .9); flat(ellPts(-s * .36, -s * .36, s * .07, s * .09, 8), C.ink); }
      pop();
    }
  }
  ...
  function hourglass(x, y, s, rot, k) {
    ...
    fillInk(rrPts(-s * .62, -s * 1.08, s * 1.24, s * .2, s * .06), '#8C6A45', C.ink, .9);
    fillInk(rrPts(-s * .62, s * .88, s * 1.24, s * .2, s * .06), '#8C6A45', C.ink, .9);
    fillInk([[-s * .5, -s * .88], [s * .5, -s * .88], [s * .08, 0], [s * .5, s * .88], [-s * .5, s * .88], [-s * .08, 0]], '#3A4150', C.ink, .9);
    const top = 1 - k, bot = k, sand = '#E7B447';
    if (top > .02) flat([[-s * .42 * top, -s * .72 * top], [s * .42 * top, -s * .72 * top], [s * .06, -s * .05], [-s * .06, -s * .05]], sand);
    if (bot > .02) flat([[-s * .44, s * .86], [s * .44, s * .86], [s * .4 * (1 - bot * .5), s * .86 - s * .7 * bot], [-s * .4 * (1 - bot * .5), s * .86 - s * .7 * bot]], sand);
  ...
  function crumbleCoin(x, y, r, t) {
    ...
    for (let i = 0; i < 6; i++) {   // six wedges fall, spin and grey out
      const a0 = i / 6 * TAU + .3, am = a0 + TAU / 12, g = clamp(p / .45), burst = (120 + 90 * hash(i)) * (1 - Math.exp(-p * 9)) / 9 * 3;
      const cx = x + Math.cos(am) * burst, cy = y + Math.sin(am) * burst * .7 + 1100 * p * p, rot = (hash(i + 3) - .5) * 9 * p, col = mixCol(C.gold, '#6E6A66', g), sc = 1 - .45 * g;
      const a1 = a0 + TAU / 6;
      if (p > .75) continue;
      const P = [[0, 0]]; for (let k = 0; k <= 3; k++) { const a = lerp(a0, a1, k / 3); P.push([Math.cos(a) * r * sc, Math.sin(a) * r * sc]); }
      boilSeed('shard' + i);
      push(); translate(cx, cy); rotate(rot); fillInk(P, col, g < .6 ? C.goldDk : null, .8, 255 * (1 - seg(p, .45, .75))); pop();
    }
    for (let i = 0; i < 16; i++) {  // the dust
      const q = clamp((p - .015 * i) / 1.1); if (q <= 0 || q >= 1) continue;
      const ang = hash(i + 20) * TAU, d = (30 + 80 * hash(i + 21)) * easeOut(q);
      boilSeed('dust' + i);
      flat(ellPts(x + Math.cos(ang) * d, y + Math.sin(ang) * d * .6 + 90 * q * q - 40 * q, 9 + 20 * q, 7 + 14 * q, 9, 1.5), i % 3 ? '#8E8983' : '#B5A98C', 190 * (1 - q));
    }
  }
  ...
  const coinsK = t => backOut(seg(t, E.coins, E.coins + .22));
  ...
  function hop(t) {
    ...
    const c1 = camParams(E.land), s1 = toScreen(DESK.youX, DESK.top, c1), u1 = 21 * c1.zoom, k = ease(seg(t, E.pull, E.land)), air = Math.sin(Math.PI * k);
  ...
  function shotI(t, lt, dur) {
    ...
    const set = { room: t >= E.pull, tl: t - END, card: cardSt, cardFlip: flipK, upGlow: ease(seg(t, E.flip, E.flip + .25)), tickGlow: .6,
      fn: HOOK0.btc, lo: HOOK0.lo, hi: HOOK0.hi, named: 0, lineK: ease(seg(t, E.line, E.lineEnd)) };
    ...
    HI.inH(() => {
      ...
      if (gone > 0) {   // the old line and its average break into ash that drifts up and away
        for (let i = 0; i < 36; i++) {
          ...
          boilSeed('cash' + i);
          flat(ellPts(x + 20 * Math.sin(i + q * 3), y - 140 * q * (.5 + hash(i + 2)), 5 + 4 * hash(i + 4), 4, 7, .8, i), i % 2 ? '#9A958C' : '#7C84B8', 220 * (1 - q));
        }
      }
      // the locked point: a ring clamps on at the close, then it slides left
      if (t > E.lock - .16 && dotA > 0) {
        const ring = t < E.lock ? lerp(90, 26, easeIn(seg(t, E.lock - .16, E.lock))) : 22 + 8 * Math.exp(-(t - E.lock) * 9) * Math.cos((t - E.lock) * 30);
        const fl = t > E.lock ? Math.exp(-(t - E.lock) * 5) : 0;
        glow(dotX, TW0, 110 + 260 * fl, '#8C9BFF', (.7 + .6 * fl) * dotA);
        boilSeed('lock');
        fillInk(ellPts(dotX, TW0, 13 * dotA, 13 * dotA, 16), t > E.lock ? '#DDE2FF' : '#AEB8FF', C.twapDk, 1.2);
        { const R = ellPts(dotX, TW0, ring * dotA, ring * dotA, 20); inkLine(R.concat([R[0], R[1]]), 2.2, C.cream, 'ink', .8); }
      }
      ...
      if (t < E.pull) {
        ...
        if (t < E.chomp) { if (ck > .02) coinLite(tipR[0] + 8, tipR[1] - 8, 42 * ck, 'r'); }
        else if (t < E.jawOut + .5) coinLite(away[0] - 44, away[1] - 4, 42, 'r');
        if (t > E.jawIn && t < E.jawOut + .5) {
          ...
          jaw(x, y - 4, 80, clamp(open + bite), t);
          if (t > E.chomp && t < E.chomp + .5) for (let i = 0; i < 4; i++) {   // gold crumbs
            const p = t - E.chomp; boilSeed('crumb' + i);
            flat(ellPts(grab[0] - 20 + 14 * i, grab[1] + 10 + 400 * p * p + 30 * p * hash(i), 6, 5, 6), C.gold, 255 * (1 - p * 2));
          }
        }
        // left coin: the last one
        if (t < E.crumble + 1 && ck > .02) crumbleCoin(tipL[0] - 8, tipL[1] - 8, 42 * ck, t);
      }
    });
    ...
    if (gk > 0) {
      const px = 400, py = 330;
      HI.hPill('0.5 s', px + 22, py, 60, { k: gk, w: 280, dx: 32, bg: C.panelHi, key: 'delay' });
      HI.inH(() => hourglass(px - 68, py, 27 * backOut(gk), Math.PI * easeOut(seg(t, E.glass, E.glass + .22)), ease(seg(t, E.glass + .2, E.glass + .7))));
    }

    // ---- the new line: the close stretches into the next round's Price to Beat ----
    const Yd = HY + TW0 / HS, dashK = j => 1 - ease(seg(t, E.dashOut + .012 * (20 - j), E.dashOut + .2 + .012 * (20 - j)));
    if (t > E.slide) {
      ...
      for (let i = 0; i < n; i += 2) {
        ...
        inkLine([[m - h, Yd], [m + h, Yd]], 1.5 * clamp(1.6 / CAM.zoom, .3, 1), '#AEB8FF', 'ink', 0);
      }
    }
